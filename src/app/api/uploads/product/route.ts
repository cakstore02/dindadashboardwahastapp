...
export async function POST(request: NextRequest) {
  ...
  const timestamp = Math.floor(Date.now() / 1000); const folder = process.env.CLOUDINARY_PRODUCT_FOLDER || "softberystore/products";
...
export async function DELETE(request: NextRequest) {
  ...
  const folder = process.env.CLOUDINARY_PRODUCT_FOLDER || "softberystore/products";
  if (typeof publicId !== "string" || !publicId.startsWith(`${folder}/`)) return NextResponse.json({ error: "Public ID gambar tidak valid." }, { status: 400 });
  const cloud = process.env.CLOUDINARY_CLOUD_NAME; const key = process.env.CLOUDINARY_API_KEY; const secret = process.env.CLOUDINARY_API_SECRET;
  if (!cloud || !key || !secret) return NextResponse.json({ error: "Cloudinary belum dikonfigurasi." }, { status: 503 });
